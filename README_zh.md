...
把**看到的（图片/截图）**、**听到的（录音）**、**读到的（网页链接）**、**想到的（文字）**随手丢进时光机，AI 自动整理成带时间戳的条目，并支持自然语言模糊检索。
...
- **录入**：随手记文字、丢图片（⌘V 粘贴或一次多张）、录音，或粘贴**网页链接**：自动抓取网页、把正文存到本地（防止链接失效，且全文可被检索），AI 还会按界面语言生成摘要——界面是中文时，英文文章也会得到中文摘要，原文保留在下方。可顺手加标签；条目后台 AI 整理，往年的会以**"那年今日"**浮现。
